...
// 创建一个非泛型接口
public interface IPushContentBll
{
    int IType { get; }
    string Title { get; }
    (string, long) AddData(PushInfo model);
    Task StartPush(long Id, Action<string, int, int> CallBack);

    void OnOpenAdd(long Id);
    SiteSelector NewSiteSelector();
    string ImportData(List<ImportModel> importModels);
    List<PushInfo> Search(int status = -1, string keyword = "", int ClassId = 0);

    void Delete(long id);
    void Update(PushInfo model);

    PushInfo GetEntity(long id);

    //List<PushInfo> GetScheduledContents(int limit = 100);

    //void UpdateStatusToPush(PushInfo model);
    //bool IsHave(string video_path);
    // 添加其他需要公开的方法
}

abstract public class PushContentBllBase<TPush, ISiteSelector> : LiteDbInt<PushInfo> , IPushContentBll
    where TPush : PushBase
    where ISiteSelector : SiteSelector, new()
{
    ...
    public void OnOpenAdd(long Id)
    {
        var model = GetEntity(Id);
        var addWin = CreateAddWindow(model);
    ...
    public Task StartPush(long Id, Action<string, int, int> CallBack)
    {
        return PushTask(Id, CallBack);
    }
    protected abstract XsDockContent CreateAddWindow(PushInfo model);

    //abstract public Task StartPush(long Id, Action<string, int, int> CallBack);
    //abstract public SiteSelector NewSiteSelector();
    public SiteSelector NewSiteSelector()
    {
        return new ISiteSelector();
    }
    ...
    protected async Task PushTask(long Id, Action<string, int, int> CallBack) 
    {
        ...
        try
        {
            ...
            {
                var pushPlugin = PluginUtils.GetByClasName<TPush>(site.Key);
    ...
    public void Update(PushInfo model)
    {
        base.Update(model);
    }

    public PushInfo GetEntity(long id)
    {
