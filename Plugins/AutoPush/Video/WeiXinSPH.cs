...
namespace SmartMedia.Plugins.AutoPush.Video
{
    public class WeiXinSPH : VideoPushBase
    {
        ...
        /// 声明原创 - 修复版本
        /// </summary>
        private async Task<bool> DeclareOriginal(IPage page)
        {
            try
            {
                PrintLog("开始声明原创流程");
                ...
                // 方法1：通过文本标签点击（最可靠）
                PrintLog("尝试通过文本标签点击");
                var label = page.GetByText("声明后，作品将展示原创标记，有机会获得广告收入。");
                ...
                // 最简洁且安全的做法
                //await page.Locator(".original-proto-wrapper input[type='checkbox']").CheckAsync();
                await page.Locator(".original-proto-wrapper input[type='checkbox']").Nth(0).CheckAsync();

                //// 按钮可能因复选框未勾选而禁用，现在应该可用了
                //var declareButton = page.Locator("button.weui-desktop-btn_primary:has-text('声明原创')");
                //await declareButton.WaitForAsync(new LocatorWaitForOptions
                //{
                //    State = WaitForSelectorState.Visible,
                //    Timeout = 5000
                //});
                ...
                //// 检查按钮是否可用
                //var isDisabled = await declareButton.GetAttributeAsync("disabled");
                //if (isDisabled != null)
                //{
                //    PrintLog("警告: 声明原创按钮仍处于禁用状态");
                //}

                //await declareButton.ClickAsync();
                //PrintLog("声明原创按钮已点击");
